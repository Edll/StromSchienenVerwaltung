...
import de.edlly.werkstueck.Konstanten;

/**
 * KoordinatenLoch Das Werkstück ist in XY Koordinaten aufgeteilt. Dabei ist X ist die feste Material Seite. (20x10,
 * 30x10...)
 * 
 * Attribute: int x, y: Die übergebenen Attribute x,y sind dabei die zu übergebenen Werte des Aktuellen Lochs int xMax:
 * Mit xMax wird die feste Material Seite übergeben. (20, 30, 40.....). Ist diese 0 wird x nicht geprüft. int
 * Durchmesser: Durchmesser ist der Durchmesser des Aktuellen Lochs. int BohrId: Die ID der zu prüfenden Bohrung. Ist
 * diese 0 wird nicht gepr�ft.
 * 
 * Methoden: CheckXY() Prüft ob die angegeben x, y werte noch auf dem Werkstück liegen. CheckLoch() Prüft ob das Loch
 * Innerhalb der Begrenzungen liegt. CheckKollision() Prüft ob die Bohrung mit einer Biegung im Datensatz Kollidiert.
 * 
 * 
 * HilfsFunktionen CheckKoord() Vergleicht den eingebenden wert mit einander
 * 
 * @author Edlly
 *
 */
public class KoordinatenLoch {
    ...
    /**
     * Prüft ob die Werte noch auf dem Werkst�ck liegen Ist das Klassen Attribut xMax 0 oder die Konstante
     * MATERIAL_MAX_LAENGE 0 wird nicht geprüft und es gibt einen throw.
     * 
	...
	if (KonstatenCheck == yMax) {
	    throw new IllegalArgumentException(
		    "yMax ist 0, pr�fung nicht möglich. Bitte die Einstellungen überprüfen.");
	}
    ...
    /**
     * CheckBohrung() Prüft ob die Bohrung noch auf dem Werkstück liegt.
     * 
     * @return true alle werte Okay
     */
	...
	if ((this.x + MinLochAbstand) > this.xMax | (MinLochAbstand) > this.x) {
	    throw new IllegalArgumentException(
		    "Das Loch ist zu nah am Rand auf der X Achse. Minimaler Abstand ist: " + (MinLochAbstand));
	}

	// X Pr�fen der Randlage des Loch
	if ((this.y + MinLochAbstand) > this.yMax | (MinLochAbstand) > this.y) {
	    throw new IllegalArgumentException(
		    "Das Loch ist zu nah am Rand auf der Y Achse. Minimaler Abstand ist: " + (MinLochAbstand));
	}
