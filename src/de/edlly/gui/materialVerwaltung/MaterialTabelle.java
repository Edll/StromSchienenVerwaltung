...
/**  material.Manager
 * Stellt die Anzeige, Tabelle, und Abfragen für den Material Manager zu Verfügung.
 * 
 * Attribute:
 * Connection SqlConn: Database Connection aus aufgerufenen Klasse
 * 
 * UnterKlassen:
 * ManagerModel das Tabellen Render Model für die Ausgabe Table
 * 
 * Methoden:
 * AusgabeTable(): gibt die Darstellung einer
 * 
 * @author Edlly
 *
 */
...
public class MaterialTabelle  {
	...
	ManagerModel  JTableWerkstueckWerteModel = new ManagerModel();
	 ...
	 for(int i=0; i!=Arraylength; i++){
		 ...
		 JTableWerkstueckWerteModel.addRow(new Object[]{MaterialDBData[i][0],MaterialDBData[i][1],MaterialDBData[i][2], MaterialDBData[i][3], checkbox}); 
		}
	 }
	              ...
	              for(int i=0;i<JTableWerkstueckWerte.getRowCount();i++)
	                {
	                   ...
	                   {  
	                	 ...
	                	 int id = Integer.parseInt((String) JTableWerkstueckWerte.getValueAt(JTableWerkstueckWerte.getSelectedRow(),0));
	                	 int value = 0;
	...
	// Eigenes TableModel um die Zellen nicht Editierbar zu machen und Boolean für die Checkbox zu erzwingen.
